...
export const CONDUCTOR_MAX_TEMP_C: Record<InsulationType, number> = {
  PVC:  70,    // °C
  XLPE: 90,    // °C
  EPR:  90,    // °C
};

// ─── Impedancia de la red (datos compañía) ────────────────────────────────

export interface NetworkImpedanceData {
  description: string;
  maxIccKA: number;       // Corriente de cortocircuito máxima en la CGP (kA)
  networkImpedanceOhm: number; // Impedancia de la red hasta la CGP
}

// Valores típicos para España (Iberdrola, Endesa, etc.)
export const TYPICAL_NETWORK_IMPEDANCE: Record<string, NetworkImpedanceData> = {
  urban_residential: {
...
// ─── Cálculo de impedancia del bucle de defecto ───────────────────────────

export interface LoopImpedanceInput {
  sectionMm2: number;           // Sección del conductor de fase (mm²)
  protectionSectionMm2: number; // Sección del conductor PE (mm²)
  lengthM: number;              // Longitud del tramo (m)
  material: ConductorMaterial;
  insulation: InsulationType;
  upstreamImpedanceOhm?: number; // Impedancia aguas arriba (default 0.038Ω red urbana)
}

export interface LoopImpedanceResult {
  loopImpedanceOhm: number;     // Zs total (fase + PE + red)
  phaseImpedanceOhm: number;    // Resistencia fase del tramo
  peImpedanceOhm: number;       // Resistencia PE del tramo
  IccMaxA: number;              // Icc máxima en el origen del tramo
  IccMinA: number;              // Icc mínima al final del tramo (bucle completo)
  IccMinKA: number;
}
 ...
 *   Icc_min = U0 / Zs
 */
export function calculateLoopImpedance(input: LoopImpedanceInput): LoopImpedanceResult {
  const tempC = CONDUCTOR_MAX_TEMP_C[input.insulation];
  const rho = getResistivityAtTemp(input.material, tempC);
  ...
  const U0 = NETWORK_VOLTAGE.phase_neutral;
...
export interface IndirectContactVerification {
  ...
  isDifferentialTriggered: boolean;   // ¿El diferencial actúa?
  isProtected: boolean;
  warnings: string[];
}
 ...
 */
export function verifyIndirectContactProtection(params: {
  earthResistanceOhm: number;        // Resistencia de puesta a tierra
  differentialSensitivityMa: number; // Sensibilidad diferencial (mA)
  ...
  if (!isProtected) {
    warnings.push(
      `ITC-BT-24: Tensión de contacto ${Uc.toFixed(1)}V supera ${maxUc}V. ` +
  ...
  if (params.earthResistanceOhm > 1667 && params.differentialSensitivityMa === 30) {
    warnings.push(
      `ITC-BT-24: Con diferencial 30mA, R_tierra ≤ 1.666Ω para Uc ≤ 50V. Valor actual: ${params.earthResistanceOhm.toFixed(1)}Ω`
    );
  }
