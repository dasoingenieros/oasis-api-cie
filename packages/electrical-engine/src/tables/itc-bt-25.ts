...
export interface CircuitTemplate {
  ...
  // Potencia de diseño por punto / total prevista (W)
  powerPerPointW: number;
  minPoints: {
    studio: number;      // Estudio < 30 m²
    small: number;       // Vivienda pequeña 30–49 m²
    medium: number;      // Vivienda media 50–99 m²
    large: number;       // Vivienda grande 100–149 m²
    xlarge: number;      // Vivienda grande >150 m²
  };
...
export const ITC_BT_25_CIRCUITS: Record<string, CircuitTemplate> = {
 ...
 */
export function getCircuitTemplate(code: string): CircuitTemplate | undefined {
  return ITC_BT_25_CIRCUITS[code];
}
 ...
 */
export function getMandatoryCircuits(surfaceM2: number): CircuitCode[] {
  const mandatory: CircuitCode[] = ["C1", "C2", "C3", "C4.1", "C5"];
