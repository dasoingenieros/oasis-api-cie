...
describe("ITC-BT-22 — getKConstant", () => {
  it("Cu/PVC = 115", () => {
    expect(getKConstant("Cu", "PVC")).toBe(115);
  });

  it("Cu/XLPE = 143", () => {
    expect(getKConstant("Cu", "XLPE")).toBe(143);
  });

  it("Cu/EPR = 143 (igual que XLPE)", () => {
    expect(getKConstant("Cu", "EPR")).toBe(143);
  });

  it("Al/PVC = 74", () => {
    expect(getKConstant("Al", "PVC")).toBe(74);
  });

  it("Al/XLPE = 94", () => {
    expect(getKConstant("Al", "XLPE")).toBe(94);
  });
...
describe("ITC-BT-22 — Constantes", () => {
  it("BREAKER_CHARACTERISTICS tiene curvas B, C, D", () => {
    expect(BREAKER_CHARACTERISTICS.B.curve).toBe("B");
    expect(BREAKER_CHARACTERISTICS.C.curve).toBe("C");
    expect(BREAKER_CHARACTERISTICS.D.curve).toBe("D");
  });

  it("curva C: magneticTrip 5–10×In", () => {
    expect(BREAKER_CHARACTERISTICS.C.magneticTripMin).toBe(5);
    expect(BREAKER_CHARACTERISTICS.C.magneticTripMax).toBe(10);
  });

  it("BREAKING_CAPACITY_KA incluye 6 y 25 kA", () => {
    expect(BREAKING_CAPACITY_KA).toContain(6);
    expect(BREAKING_CAPACITY_KA).toContain(25);
  });

  it("MIN_BREAKING_CAPACITY_KA por tipo de instalación", () => {
    expect(MIN_BREAKING_CAPACITY_KA.residential).toBe(6);
    expect(MIN_BREAKING_CAPACITY_KA.commercial).toBe(10);
    expect(MIN_BREAKING_CAPACITY_KA.industrial).toBe(15);
  });
});
...
describe("ITC-BT-22 — selectPIA", () => {
  it("Ib=10, Iz=20: selecciona 10A o 13A", () => {
    const result = selectPIA({ Ib: 10, Iz: 20 });
  ...
  it("IccMaxKA personalizado afecta breakingCapacityKA", () => {
    const result6 = selectPIA({ Ib: 10, Iz: 20, IccMaxKA: 6 });
    const result15 = selectPIA({ Ib: 10, Iz: 20, IccMaxKA: 15 });
  ...
  it("condición 2 fallida genera warning", () => {
    ...
    const result = selectPIA({ Ib: 60, Iz: 63 });
    ...
    const result2 = selectPIA({ Ib: 55, Iz: 40 });
  ...
  it("devuelve condition1 y condition2", () => {
    const result = selectPIA({ Ib: 10, Iz: 25 });
...
describe("ITC-BT-22 — verifyShortCircuitCapacity", () => {
  it("I²t ≤ K²×S²: isCompliant true", () => {
    const result = verifyShortCircuitCapacity({
      IccA: 1000,
