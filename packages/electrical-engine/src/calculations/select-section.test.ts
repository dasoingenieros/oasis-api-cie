...
describe("selectSection — criterio térmico determinante", () => {
  it("C1 alumbrado 2300W: sección 1.5mm² por térmico", () => {
    ...
    const result = selectSection(
      makeCircuitInput({ code: "C1", loadPowerW: 2300 })
    );
  ...
  it("con agrupamiento Cg=0.7 requiere mayor sección térmica", () => {
    const result1 = selectSection(
      makeCircuitInput({ groupingCircuits: 1, loadPowerW: 3500 })
    );
    const result7 = selectSection(
      makeCircuitInput({ groupingCircuits: 7, loadPowerW: 3500 })
    );
  ...
  it("devuelve thermalSectionMm2 correcto", () => {
    const result = selectSection(makeCircuitInput({ loadPowerW: 2300 }));
...
describe("selectSection — criterio caída de tensión determinante", () => {
  ...
  it("devuelve voltageDropSectionMm2 correcto", () => {
    const result = selectSection(
      makeCircuitInput({ loadPowerW: 2300, lengthM: 20 })
    );
    expect(result.voltageDropSectionMm2).toBeGreaterThanOrEqual(1.5);
...
describe("selectSection — criterio ITC-BT-25 determinante", () => {
  it("C1 tiene mínimo 1.5mm² por ITC-BT-25", () => {
    const result = selectSection(
      makeCircuitInput({ code: "C1", loadPowerW: 500 }) // Muy poca potencia
    );
...
describe("selectSection — máximo de los tres criterios", () => {
  it("devuelve la mayor de las tres secciones", () => {
    ...
    const maxOfThree = Math.max(
      result.thermalSectionMm2,
      result.voltageDropSectionMm2,
      result.minimumItcBt25SectionMm2 ?? 0
    );
...
describe("selectSection — NUNCA NaN, siempre válido", () => {
  it("nunca devuelve NaN", () => {
    ...
    for (const input of inputs) {
      const result = selectSection(input);
  ...
  it("determinantCriteria siempre es uno de los tres", () => {
    ...
    const result = selectSection(makeCircuitInput());
...
describe("selectSection — factores de corrección", () => {
  it("temperatura ambiente alta reduce Iz efectiva", () => {
    const result25 = selectSection(
      makeCircuitInput({ ambientTempC: 25, loadPowerW: 3500 })
    );
    const result50 = selectSection(
      makeCircuitInput({ ambientTempC: 50, loadPowerW: 3500 })
    );
  ...
  it("aplica simultaneidad y factor de utilización a In", () => {
    const resultFull = selectSection(
      makeCircuitInput({ loadPowerW: 3450, simultaneityFactor: 1, loadFactor: 1 })
    );
    const resultReduced = selectSection(
      makeCircuitInput({
        loadPowerW: 3450,
...
describe("selectSection — métodos de instalación", () => {
  it("método A1 (empotrado aislante) da mayor sección que B1 para misma In", () => {
    ...
    expect(resultA1.thermalSectionMm2).toBeGreaterThanOrEqual(
      resultB1.thermalSectionMm2
    );
  });
  ...
  it("XLPE permite mayor Iz que PVC para misma sección", () => {
    ...
    const resultXLPE = selectSection(
      makeCircuitInput({
        insulationType: "XLPE",
        loadPowerW: 4000,
        lengthM: 5,
    ...
    expect(resultXLPE.thermalSectionMm2).toBeLessThanOrEqual(
      resultPVC.thermalSectionMm2
    );
  });
});
