...
describe("ITC-BT-14 — Constantes", () => {
  it("RESISTIVITY_20C tiene valores para Cu y Al", () => {
    expect(RESISTIVITY_20C.Cu).toBeCloseTo(0.01724, 4);
    expect(RESISTIVITY_20C.Al).toBeCloseTo(0.02826, 4);
  });

  it("TEMP_COEFF tiene valores para Cu y Al", () => {
    expect(TEMP_COEFF.Cu).toBeCloseTo(0.00393, 4);
    expect(TEMP_COEFF.Al).toBeCloseTo(0.00403, 4);
  });

  it("LGA_SECTION_TABLE tiene 10 filas", () => {
    expect(LGA_SECTION_TABLE).toHaveLength(10);
  });

  it("LGA_SECTION_TABLE primera fila: 100A → 16mm² Cu, 25mm² Al", () => {
    const first = LGA_SECTION_TABLE[0];
  ...
  it("REACTANCE_TABLE_CABLE contiene secciones ≥ 50mm²", () => {
    expect(REACTANCE_TABLE_CABLE[50]).toBe(0.09);
    expect(REACTANCE_TABLE_CABLE[300]).toBe(0.072);
    expect(REACTANCE_TABLE_CABLE[25]).toBeUndefined();
  });
});
...
describe("ITC-BT-14 — getResistivityAtTemp", () => {
  it("Cu a 20°C devuelve ρ_20", () => {
    const rho = getResistivityAtTemp("Cu", 20);
    expect(rho).toBeCloseTo(RESISTIVITY_20C.Cu, 6);
  });

  it("Al a 20°C devuelve ρ_20", () => {
    const rho = getResistivityAtTemp("Al", 20);
    expect(rho).toBeCloseTo(RESISTIVITY_20C.Al, 6);
  });

  it("Cu a 70°C tiene mayor resistividad que a 20°C", () => {
    const rho20 = getResistivityAtTemp("Cu", 20);
    const rho70 = getResistivityAtTemp("Cu", 70);
    expect(rho70).toBeGreaterThan(rho20);
  });

  it("Al a 90°C tiene mayor resistividad que a 25°C", () => {
    const rho25 = getResistivityAtTemp("Al", 25);
    const rho90 = getResistivityAtTemp("Al", 90);
    expect(rho90).toBeGreaterThan(rho25);
  });

  it("fórmula ρ_T = ρ_20 × [1 + α × (T - 20)] para Cu a 70°C", () => {
    const rho = getResistivityAtTemp("Cu", 70);
    const expected = RESISTIVITY_20C.Cu * (1 + TEMP_COEFF.Cu * 50);
    expect(rho).toBeCloseTo(expected, 6);
  });

  it("nunca devuelve NaN", () => {
    expect(isNaN(getResistivityAtTemp("Cu", 0))).toBe(false);
    expect(isNaN(getResistivityAtTemp("Al", 100))).toBe(false);
  });
});
...
describe("ITC-BT-14 — calculateLGAVoltageDrop", () => {
  it("trifásico: calcula In = P / (√3 × V × cosφ)", () => {
    const result = calculateLGAVoltageDrop({
      totalPowerW: 34500,
      powerFactor: 0.9,
      phaseSystem: "three",
  ...
  it("sin advertencias cuando secciones cumplen mínimo", () => {
    const resultCu = calculateLGAVoltageDrop({
      totalPowerW: 5000,
      powerFactor: 0.9,
      phaseSystem: "three",
      conductorMaterial: "Cu",
    ...
    const resultAl = calculateLGAVoltageDrop({
      totalPowerW: 5000,
      powerFactor: 0.9,
      phaseSystem: "three",
      conductorMaterial: "Al",
  ...
  it("conductorTempC personalizado afecta resistividad", () => {
    const result70 = calculateLGAVoltageDrop({
      totalPowerW: 10000,
      powerFactor: 0.9,
      phaseSystem: "three",
      conductorMaterial: "Cu",
    ...
    const result90 = calculateLGAVoltageDrop({
      totalPowerW: 10000,
      powerFactor: 0.9,
      phaseSystem: "three",
      conductorMaterial: "Cu",
...
describe("ITC-BT-14 — getNeutralSection", () => {
  it("Cu: fase ≤ 16mm² → neutro = fase", () => {
    expect(getNeutralSection(1.5, "Cu")).toBe(1.5);
    expect(getNeutralSection(2.5, "Cu")).toBe(2.5);
    expect(getNeutralSection(10, "Cu")).toBe(10);
    expect(getNeutralSection(16, "Cu")).toBe(16);
  });

  it("Cu: fase > 16mm² → neutro = fase/2, mínimo 16mm²", () => {
    expect(getNeutralSection(25, "Cu")).toBe(16); // 25/2=12.5 < 16
    expect(getNeutralSection(35, "Cu")).toBe(17.5); // 35/2=17.5 → max(17.5,16)=17.5
    expect(getNeutralSection(50, "Cu")).toBe(25);
    expect(getNeutralSection(95, "Cu")).toBeCloseTo(47.5, 1);
  });

  it("Al: fase ≤ 25mm² → neutro = fase", () => {
    expect(getNeutralSection(16, "Al")).toBe(16);
    expect(getNeutralSection(25, "Al")).toBe(25);
  });

  it("Al: fase > 25mm² → neutro = fase/2, mínimo 16mm²", () => {
    expect(getNeutralSection(35, "Al")).toBe(17.5); // 35/2=17.5, max(17.5,16)=17.5
    expect(getNeutralSection(50, "Al")).toBe(25);
  });

  it("nunca devuelve valor negativo ni NaN", () => {
    expect(getNeutralSection(10, "Cu")).toBeGreaterThan(0);
    expect(getNeutralSection(120, "Al")).toBeGreaterThan(0);
    expect(isNaN(getNeutralSection(70, "Cu"))).toBe(false);
  });
});
