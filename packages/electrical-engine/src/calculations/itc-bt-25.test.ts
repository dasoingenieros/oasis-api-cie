...
describe("ITC-BT-25 — getCircuitTemplate", () => {
  ...
  it("devuelve plantilla para C4.1, C4.2, C4.3", () => {
    expect(getCircuitTemplate("C4.1")?.minSectionMm2).toBe(4);
    expect(getCircuitTemplate("C4.2")?.minSectionMm2).toBe(4);
    expect(getCircuitTemplate("C4.3")?.minSectionMm2).toBe(4);
  });

  it("devuelve plantilla para C5, C6, C7", () => {
    expect(getCircuitTemplate("C5")?.minSectionMm2).toBe(2.5);
    expect(getCircuitTemplate("C6")?.minSectionMm2).toBe(1.5);
    expect(getCircuitTemplate("C7")?.minSectionMm2).toBe(2.5);
  });

  it("devuelve plantilla para C8, C9, C10", () => {
    expect(getCircuitTemplate("C8")?.minSectionMm2).toBe(6);
    expect(getCircuitTemplate("C9")?.minSectionMm2).toBe(6);
    expect(getCircuitTemplate("C10")?.minSectionMm2).toBe(4);
  });

  it("devuelve plantilla para C11 (domótica, diferencial 300mA)", () => {
    const t = getCircuitTemplate("C11");
    expect(t).toBeDefined();
  ...
  it("devuelve plantilla para C12", () => {
    const t = getCircuitTemplate("C12");
    expect(t).toBeDefined();
    expect(t?.minSectionMm2).toBe(2.5);
  });

  it("devuelve undefined para código inexistente", () => {
    expect(getCircuitTemplate("C99")).toBeUndefined();
    expect(getCircuitTemplate("X1")).toBeUndefined();
    expect(getCircuitTemplate("")).toBeUndefined();
  });

  it("CUSTOM no está en tabla (undefined)", () => {
    expect(getCircuitTemplate("CUSTOM")).toBeUndefined();
  });
...
describe("ITC-BT-25 — getMandatoryCircuits", () => {
  ...
  it("superficie 50m²: añade C4.3 y C10", () => {
    const circuits = getMandatoryCircuits(50);
    expect(circuits).toContain("C4.2");
  ...
  it("superficie 99m²: incluye todos los obligatorios", () => {
    const circuits = getMandatoryCircuits(99);
...
describe("ITC-BT-25 — ITC_BT_25_CIRCUITS", () => {
  it("contiene al menos 13 circuitos definidos", () => {
    const keys = Object.keys(ITC_BT_25_CIRCUITS);
    expect(keys.length).toBeGreaterThanOrEqual(13);
  });

  it("cada plantilla tiene campos requeridos", () => {
    for (const [code, t] of Object.entries(ITC_BT_25_CIRCUITS)) {
  ...
  it("minSectionMm2 son valores normalizados", () => {
    ...
    for (const t of Object.values(ITC_BT_25_CIRCUITS)) {
