...
import { calculateInstallation } from "./calculate-installation";
import type { CircuitInput } from "../types";

function makeCircuitInput(
  id: string,
...
describe("calculateInstallation", () => {
  ...
  it("array vacío: circuits vacío, summary en 0", async () => {
    const result = await calculateInstallation([]);
...
describe("calculateInstallation — resumen global", () => {
  ...
  it("maxVoltageDropPct es la mayor CdT acumulada", async () => {
    ...
    const maxFromCircuits = Math.max(
      ...result.circuits.map((c) => c.accumulatedCdtPct)
    );
...
describe("calculateInstallation — múltiples circuitos", () => {
  ...
  it("usa Promise.all para procesar en paralelo", async () => {
    const inputs = Array.from({ length: 5 }, (_, i) =>
      makeCircuitInput(`c${i}`, { loadPowerW: 2300 + i * 100 })
    );
    const result = await calculateInstallation(inputs);
