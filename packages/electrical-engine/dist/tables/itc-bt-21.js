...
exports.TUBE_DIAMETER_TABLE = exports.NORMALIZED_TUBE_DIAMETERS_MM = void 0;
exports.getMinTubeDiameter = getMinTubeDiameter;
exports.getConductorCountForCircuit = getConductorCountForCircuit;
exports.getMinBendRadius = getMinBendRadius;
exports.getMinTubeDiameterForMixedConductors = getMinTubeDiameterForMixedConductors;
// ─── Diámetros exteriores normalizados de tubo (mm) ──────────────────────
exports.NORMALIZED_TUBE_DIAMETERS_MM = [16, 20, 25, 32, 40, 50, 63];
/**
 ...
 */
function getMinTubeDiameter(sectionMm2, numConductors) {
    const entry = exports.TUBE_DIAMETER_TABLE[sectionMm2];
    if (!entry) {
        // Para secciones no tabuladas, buscar la sección superior más cercana
        const sections = Object.keys(exports.TUBE_DIAMETER_TABLE).map(Number).sort((a, b) => a - b);
 ...
 */
function getConductorCountForCircuit(params) {
    let count = params.phaseSystem === "three" ? 3 : 1;
    if (params.includeNeutral)
        count += 1;
    if (params.includeProtection)
        count += 1;
    // Clamp a 1-5
 ...
 */
function getMinBendRadius(tubeDiameterMm) {
