...
exports.MIN_BREAKING_CAPACITY_KA = exports.BREAKING_CAPACITY_KA = exports.BREAKER_CHARACTERISTICS = exports.K_CONSTANT = void 0;
exports.getKConstant = getKConstant;
exports.selectPIA = selectPIA;
exports.verifyShortCircuitCapacity = verifyShortCircuitCapacity;
exports.checkSelectivity = checkSelectivity;
...
function getKConstant(material, insulation) {
    const key = `${material}_${insulation}`;
    return exports.K_CONSTANT[key] ?? 115; // fallback conservador Cu/PVC
}
exports.BREAKER_CHARACTERISTICS = {
    B: {
        curve: "B",
        magneticTripMin: 3,
        magneticTripMax: 5,
...
exports.BREAKING_CAPACITY_KA = [1.5, 3, 4.5, 6, 10, 15, 20, 25];
// Poder de corte mínimo por tipo de instalación (ITC-BT-22 §3)
exports.MIN_BREAKING_CAPACITY_KA = {
 ...
 */
function selectPIA(input) {
    const curve = input.curve ?? "C";
    const warnings = [];
    // Buscar calibre mínimo ≥ Ib
    let selectedRating;
    for (const rating of types_1.NORMALIZED_BREAKER_RATINGS) {
    ...
    if (selectedRating === undefined) {
        ...
        selectedRating = types_1.NORMALIZED_BREAKER_RATINGS.find(r => r >= input.Ib) ?? types_1.NORMALIZED_BREAKER_RATINGS[types_1.NORMALIZED_BREAKER_RATINGS.length - 1];
        warnings.push(`ITC-BT-22: No existe calibre normalizado que cumpla ${input.Ib.toFixed(1)}A ≤ In ≤ ${input.Iz.toFixed(1)}A. ` +
            `Aumentar sección del conductor o reducir la carga.`);
    }
    const I2 = selectedRating * exports.BREAKER_CHARACTERISTICS[curve].I2factor;
    ...
    const breakingCapacity = exports.BREAKING_CAPACITY_KA.find(bc => bc >= IccMax) ?? 25;
    if (!cond2Ok) {
 ...
 */
function verifyShortCircuitCapacity(params) {
 ...
 */
function checkSelectivity(params) {
...
//# sourceMappingURL=itc-bt-22.js.map